    ...
    try {
      ...
      for (const client of clients) {
        try {
          ...
          const display = baseDisplayName(client);
            ...
            result = await limiter.schedule(
              () =>
                new Promise((resolve, reject) => {
                  qb.createCustomer(buildPayload(display), (err: any, data: any) => (err ? reject(err) : resolve(data)));
                })
            );
          } catch (err: any) {
            // 2) se nome duplicado, tenta de novo COM sufixo
            if (isDuplicateNameError(err)) {
              const displayWithSuffix = withSuffix(display, client);
              result = await limiter.schedule(
                () =>
                  new Promise((resolve, reject) => {
                    qb.createCustomer(buildPayload(displayWithSuffix), (e: any, data: any) => (e ? reject(e) : resolve(data)));
                  })
              );
            } else {
              throw err; // outros erros seguem o fluxo normal de erro
            }
          }

          // aceita Customer “desembrulhado” ou dentro de { Customer }
