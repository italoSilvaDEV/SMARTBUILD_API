...
export class QuickBooksClientController {
  async syncClients(req: Request, res: Response) {
    ...
    try {
      ...
      // 5. Paginação correta da API QuickBooks (query e paginação manual)
      let allClients: any[] = [];
      let startPosition = 1;
      const maxResults = 1000; // QuickBooks permite até 1000 por query
      ...
      while (keepFetching) {
        ...
        allClients = allClients.concat(customers);
