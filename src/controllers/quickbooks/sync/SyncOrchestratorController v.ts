  ...
  private async executeSync(
    ...
    for (const preference of preferences) {
      ...
      try {
        ...
        if (typesEntity === 'customers' && typeSync === 'QuickBooksToSmartBuild') {
          syncResult = await this.executeCustomerSyncFromQuickBooks(companyId, userId);
        } else {
        ...
        await (prisma as any).syncStatus.update({
          ...
          data: {
            status: 'COMPLETED',
            lastSyncAt: new Date(),
            successRecords: syncResult.synced || 0,
            errorCount: 0,
            lastError: null
          }
        });
