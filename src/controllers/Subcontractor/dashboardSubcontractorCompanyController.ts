...
/**
 * Dashboard agregado de todos os subcontractors da company.
 * Mesmo formato de resposta do dashboard por subcontractor (linha + totais).
 */
export class DashboardSubcontractorCompanyController {
    async handle(req: Request, res: Response) {
        ...
        try {
            ...
            // Trabalhos de subcontractor da company (amount_of_hours null = subcontractor)
            ...
            const allProjectIds = allWorkedHours.map((r: { project_id: string | null }) => r.project_id).filter(Boolean) as string[];
            ...
            const filteredProjects = await prisma.project.findMany({
                where: {
                    ...
                    ...(shouldFilterByStatus && {
                        status_project: { in: statusFilters }
                    })
                },
                select: { id: true },
            });
            ...
            const workedHoursRecords = await prisma.workedhours.findMany({
                where: {
                    amount_of_hours: null,
                    subcontractor_id: { not: null },
                    subcontractor: { company_id: companyId },
            ...
            if (isCustom) {
                ...
                while (current.isBefore(last) || current.isSame(last)) {
                    ...
                    monthlySales.push({ month: monthKey, value: monthlyData[monthKey] || 0 });
            ...
            console.error("Error fetching company subcontractor dashboard:", error);
            return res.status(500).json({
                error: "Internal server error while fetching company subcontractor dashboard"
            });
        }
    }
}
